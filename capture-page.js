...
function onMessage(data, sender, callback) {
    if (data.msg === 'scrollPage') {
        getPositions(callback, data.static);
        return true;
...
function getPositions(callback, static){
    ...
    (function processArrangements() {
        ...
        if (!static) window.scrollTo(x, y);

        var startY = static? 0 : window.scrollY;
        var endY = static? window.innerHeight||document.documentElement.clientHeight||document.body.clientHeight
            : fullHeight;
