...
function getFilename(contentURL) {
    ...
    return 'screencapturex' + name + '-' + Date.now() + '.jpg';
}
...
chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    Capture.captureToFiles(tab, filename, displayCaptures,
...
window.Capture = (function () {
    ...
    function capture(data, screenshots, sendResponse, splitnotifier) {
        chrome.tabs.captureVisibleTab(
            null, { format: 'jpg', quality: 10 }, function (dataURI) {
                if (dataURI) {
                    var image = new Image();
    ...
    function captureToBlobs(tab, callback, errback, progress, splitnotifier) {
        ...
        chrome.tabs.executeScript(tab.id, { file: 'capture-page.js' }, function () {
            if (timedOut) {
